...
import { useCommandCentersCount, useCommandCenters } from '@/hooks/useAdmin';
import CommandCenterTable from './CommandList';
import AddCommandCenterForm from './AddCommands';


const CommandCenter: React.FC = () => {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const { data: countData, mutate: mutateCount } = useCommandCentersCount();
  const { mutate: mutateCommandCenters } = useCommandCenters();
  ...
  const handleCommandCenterAdded = () => {
  ...
  return (
    ...
    <AddCommandCenterForm isOpen={isSidebarOpen} onClose={toggleSidebar} onCommandCenterAdded={handleCommandCenterAdded} />
