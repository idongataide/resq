...
import { bulkUploadLatsma } from '@/api/lastmaApi';


const UploadLastmaMembersForm: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await bulkUploadLatsma(formData);
      if (response?.status === 'ok') {
        toast.success('Latsma uploaded successfully');
        setFile(null);
  ...
  const handleDownloadTemplate = () => {
    const headers = ['first_name', 'last_name', 'email', 'command', 'phone_number'];
    const csvContent = headers.join(',') + '\n';
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'latsma_template.csv';
...
export default UploadLastmaMembersForm; 